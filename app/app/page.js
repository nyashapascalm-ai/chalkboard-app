...
export default function Home() {
  ...
  if (checking) return <div className="center muted">LoadingÔÇª</div>;
...
function SetNewPassword({ onDone }) {
  ...
  return (<div className="center"><div className="card" style={{ maxWidth: 400 }}>
    ...
    {ok ? (<><p className="muted">Password updated.</p><button style={{ width: '100%' }} onClick={onDone}>Continue</button></>) : (<form onSubmit={save}>
      ...
      <button disabled={busy} style={{ width: '100%' }}>{busy ? 'SavingÔÇª' : 'Update password'}</button>
    </form>)}
...
function Login() {
  ...
  return (<div className="center"><div className="card" style={{ maxWidth: 400 }}>
    ...
    <p className="muted" style={{ marginTop: 0 }}>Run your school ÔÇö attendance, records and reports.</p>
    {mode === 'reset' ? (sent ? (<><p>Check your email for a reset link, then open it on this device to set a new password.</p><button className="ghost" style={{ width: '100%', marginTop: 8 }} onClick={() => { setMode('signin'); setSent(false); }}>Back to sign in</button></>) : (<form onSubmit={sendReset}>
      ...
      <button disabled={busy} style={{ width: '100%' }}>{busy ? 'SendingÔÇª' : 'Send reset link'}</button>
      ...
      <div style={{ position: 'relative' }}>
        ...
        <button type="button" onClick={() => setShowPw(v => !v)} style={{ position: 'absolute', right: 4, top: 12, background: 'transparent', border: 0, cursor: 'pointer', fontSize: 16, padding: 6 }}>{showPw ? '­ƒÖê' : '­ƒæü´©Å'}</button>
      </div>
      <button disabled={busy} style={{ width: '100%' }}>{busy ? 'Signing inÔÇª' : 'Sign in'}</button>
    </form>
...
function App({ session }) {
  ...
  if (role === undefined) return <div className="center muted">LoadingÔÇª</div>;
...
function Console({ session, role, canPick, initialSchool }) {
  ...
  useEffect(() => { (async () => {
    ...
    if (u.searchParams.get('subpay') === '1') {
      try { const res = await fetch('/api/subscription-pay/status', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ schoolId }) }); const d = await res.json(); if (d.status === 'paid') { await loadSub(); alert('Payment received ÔÇö your subscription is updated. Thank you!'); } } catch (e) {}
  ...
  const groups = [
    { key: 'academics', label: 'Academics', icon: '­ƒôÜ', items: [['attendance', 'Attendance', '­ƒôï'], ['marks', 'Marks', '­ƒôØ'], ['reportcards', 'Report cards', '­ƒôä'], ['reports', 'Attendance report', '­ƒôè']].concat(A ? [['academics', 'Class overview', '­ƒôê']] : []) },
    { key: 'people', label: 'People', icon: '­ƒæÑ', items: [['students', 'Students', '­ƒæñ']].concat(A ? [['teachers', 'Teachers', '­ƒæÑ'], ['staff', 'Staff', '­ƒæö'], ['admissions', 'Admissions', '­ƒÄô']] : []) },
    { key: 'money', label: 'Money', icon: '­ƒÆ░', items: A ? [['fees', 'Fees', '­ƒÆ░'], ['arrears', 'Arrears', 'ÔÜá´©Å'], ['finance', 'Finance', '­ƒÆÁ'], ['banking', 'Banking', '­ƒÅª']] : [] },
    { key: 'operations', label: 'Operations', icon: '­ƒùé´©Å', items: A ? [['timetable', 'Timetable', '­ƒôà'], ['inventory', 'Inventory', '­ƒôª'], ['assets', 'Assets', '­ƒÅó']] : [] },
    { key: 'comms', label: 'Communication', icon: '­ƒôú', items: [['announcements', 'Announcements', '­ƒôú']] },
    { key: 'setup', label: 'Setup', icon: 'ÔÜÖ´©Å', items: A ? [['classes', 'Classes', '­ƒÅ½'], ['subjects', 'Subjects', '­ƒôÜ'], ['school', 'School', 'ÔÜÖ´©Å']].concat(SA ? [['mybilling', 'Subscription', '­ƒÆ│']] : []) : [] },
    { key: 'platform', label: 'Platform', icon: '­ƒøí´©Å', items: O ? [['billing', 'Subscriptions', '­ƒÆ│']] : [] },
  ...
  return (<div className="shell">
    ...
    <aside className="sidebar">
      ...
      <nav className="side-nav">
        <button className={'side-item' + (nav === 'dashboard' ? ' active' : '')} onClick={() => setNav('dashboard')}><span className="si">­ƒÅá</span>Dashboard</button>
        {groups.map(g => (<div key={g.key}>
          <button style={grpHdr} onClick={() => setOpenGroup(openGroup === g.key ? null : g.key)}><span className="si">{g.icon}</span><span style={{ flex: 1 }}>{g.label}</span><span style={{ fontSize: 10 }}>{openGroup === g.key ? 'Ôû¥' : 'Ôû©'}</span></button>
      ...
      <div style={{ fontSize: 12, color: '#5b6570', padding: '8px 12px', wordBreak: 'break-all' }}>{session.user.email}{isTeacher ? ' ┬À teacher' : ''}</div>
      <button className="side-item" onClick={installApp}><span className="si">Ô¼ç</span>Download app</button>
      <button className="side-item" onClick={() => supabase.auth.signOut()} style={{ color: '#c0392b' }}><span className="si">ÔÅÅ</span>Sign out</button>
    </aside>
    <main className="main">
  ...
  return (<div>
    ...
    <div className="card" style={{ marginBottom: 18 }}>
      ...
      <div style={{ display: 'flex', gap: 10, alignItems: 'end' }}>
        ...
        <button onClick={add} disabled={busy}>{busy ? 'AddingÔÇª' : 'Add class'}</button>
      </div>
      {err && <p className="error">{err}</p>}
    </div>
    <table><thead><tr><th>Class</th><th></th></tr></thead><tbody>
      ...
      {classes.length === 0 && <tr><td colSpan="2" className="muted">No classes yet ÔÇö add one above.</td></tr>}
    </tbody></table>
  </div>);
}
...
function TeachersPanel({ schoolId, classes }) {
  ...
  return (<div>
    ...
    <div className="card" style={{ marginBottom: 18 }}>
      ...
      <div style={{ marginTop: 14 }}><button onClick={create} disabled={busy}>{busy ? 'CreatingÔÇª' : 'Create teacher login'}</button></div>
      {err && <p className="error">{err}</p>}
    </div>
...
function StudentsPanel({ schoolId, classes, isTeacher }) {
  ...
  if (classes.length === 0) return <p className="muted">{isTeacher ? 'You have no classes yet.' : 'No classes yet ÔÇö add them in the Classes tab first.'}</p>;
  ...
  return (<div>
    ...
    <div className="card" style={{ marginBottom: 18 }}>
      ...
      <div style={{ display: 'flex', gap: 10, alignItems: 'end' }}>
        ...
        <button onClick={add} disabled={busy}>{busy ? 'AddingÔÇª' : 'Add'}</button>
      </div>
      {err && <p className="error">{err}</p>}
    ...
    <div className="card" style={{ marginBottom: 18 }}>
      ...
      <div style={{ marginTop: 10 }}><button onClick={addBulk} disabled={busy}>{busy ? 'AddingÔÇª' : 'Add all (' + bulk.split('\n').map(x => x.trim()).filter(Boolean).length + ')'}</button></div>
    </div>
    <table><thead><tr><th>Name</th><th>Guardian</th><th></th></tr></thead><tbody>
      {rows.map(r => (<tr key={r.id}><td className="strong">{r.full_name}</td><td className="muted">{r.guardian_name || 'ÔÇö'}</td><td className="r"><button className="ghost" style={{ padding: '4px 10px', fontSize: 13 }} onClick={() => setEditing(r)}>Open</button></td></tr>))}
...
function StudentRecord({ student, classes, clsName, onBack, onSaved }) {
  ...
  return (<div>
    <button className="ghost" onClick={onBack} style={{ marginBottom: 14, padding: '5px 12px', fontSize: 13 }}>ÔåÉ Back to list</button>
    <div className="card">
      ...
      <div style={{ display: 'grid', gap: 12, gridTemplateColumns: '1fr 1fr' }}>
        ...
        <div><label style={labelStyle}>Class</label><select style={inputStyle} value={f.class_id} onChange={e => set('class_id', e.target.value)}><option value="">ÔÇö none ÔÇö</option>{classes.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select></div>
        <div><label style={labelStyle}>Gender</label><select style={inputStyle} value={f.gender} onChange={e => set('gender', e.target.value)}><option value="">ÔÇö</option><option value="female">Female</option><option value="male">Male</option><option value="other">Other</option></select></div>
      ...
      <div style={{ marginTop: 16, display: 'flex', gap: 10 }}>
        <button onClick={save} disabled={busy}>{busy ? 'SavingÔÇª' : (saved ? 'Saved Ô£ô' : 'Save record')}</button>
...
function AttendancePanel({ schoolId, classes, isTeacher }) {
  ...
  if (classes.length === 0) return <p className="muted">{isTeacher ? 'You have no classes yet ÔÇö add them in My classes first.' : 'No classes yet ÔÇö add them in the Classes tab first.'}</p>;
  return (<div>
    <div style={{ display: 'flex', gap: 16, alignItems: 'end', marginBottom: 16, flexWrap: 'wrap' }}>
      ...
      <div className="muted" style={{ fontSize: 14, paddingBottom: 8 }}>Present {counts.present || 0} ┬À Absent {counts.absent || 0} ┬À Late {counts.late || 0}</div>
    </div>
    {students.length === 0 ? <p className="muted">No students in this class yet. Add students in the Students tab.</p> : (<>
      ...
      <div style={{ marginTop: 18 }}><button onClick={save} disabled={busy}>{busy ? 'SavingÔÇª' : (saved ? 'Saved Ô£ô' : 'Save attendance')}</button>{err && <p className="error">{err}</p>}</div>
    </>)}
  </div>);
}
...
function letterheadHtml(school, settings) {
  ...
  const contact = [settings && settings.phone, settings && settings.email].filter(Boolean).map(esc).join(' ┬À ');
...
function ReportsPanel({ schoolId, classes, school, settings }) {
  ...
  return (<div>
    ...
    <div className="muted" style={{ marginBottom: 12, fontSize: 14 }}>{dates.length} day(s) recorded ┬À Class totals: Present {totals.present} ┬À Absent {totals.absent} ┬À Late {totals.late}</div>
    {loading ? <p className="muted">LoadingÔÇª</p> : (
    ...
    {openStudent && (<div className="card" style={{ marginTop: 16 }}>
      <div style={{ fontWeight: 700, marginBottom: 8 }}>{openStudent.full_name} ÔÇö daily record</div>
...
function SubjectsPanel({ schoolId, subjects, onChange }) {
  ...
  return (<div>
    ...
    <div className="card" style={{ marginBottom: 18 }}>
      ...
      <div style={{ display: 'flex', gap: 10, alignItems: 'end' }}><div style={{ flex: 1 }}><label style={labelStyle}>Subject name</label><input style={inputStyle} value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Mathematics" /></div><button onClick={add} disabled={busy}>{busy ? 'AddingÔÇª' : 'Add subject'}</button></div>
      {err && <p className="error">{err}</p>}
    </div>
    <table><thead><tr><th>Subject</th><th></th></tr></thead><tbody>
      ...
      {subjects.length === 0 && <tr><td colSpan="2" className="muted">No subjects yet ÔÇö add one above.</td></tr>}
    </tbody></table>
  </div>);
}
...
function MarksPanel({ schoolId, classes, subjects, teacherId, level }) {
  ...
  if (classes.length === 0) return <p className="muted">No classes available yet.</p>;
  if (subjects.length === 0) return <p className="muted">No subjects yet ÔÇö an admin adds them in the Subjects tab.</p>;
  return (<div>
    ...
    {students.length === 0 ? <p className="muted">No students in this class.</p> : (<>
      <table><thead><tr><th>Student</th><th style={{ width: 90 }}>Mark</th><th style={{ width: 60 }}>{level === 'primary' ? 'Units' : 'Grade'}</th><th>Comment</th></tr></thead><tbody>
        {students.map(s => { const row = rowData[s.id] || { score: '', comment: '' }; const g = gradeFor(row.score, level); return (<tr key={s.id}><td className="strong">{s.full_name}</td>
          ...
          <td style={{ fontWeight: 700, color: g === 'A' ? '#1a7f5a' : g === 'E' ? '#c0392b' : '#1f2328' }}>{g || 'ÔÇö'}</td>
      ...
      <div style={{ marginTop: 16 }}><button onClick={save} disabled={busy}>{busy ? 'SavingÔÇª' : (saved ? 'Saved Ô£ô' : 'Save marks')}</button>{err && <p className="error">{err}</p>}</div>
    </>)}
  </div>);
}
...
function ReportCardsPanel({ schoolId, classes, subjects, school, settings, level }) {
  ...
  function sigBlock() { return '<table style="margin-top:22px;width:100%"><tr>' + '<td style="border:0;padding-top:30px">TeacherÔÇÖs signature: ______________</td>' + '<td style="border:0;padding-top:30px">HeadÔÇÖs signature: ______________</td>' + '<td style="border:0;padding-top:30px">ParentÔÇÖs signature: ______________</td></tr></table>'; }
  ...
  const NOTE = 'NB: This report is issued without erasure/alteration and it should be returned with parent/guardianÔÇÖs signature.';
  function printFull() {
    ...
    const html = '<html><head><title>Report card</title><style>body{font-family:Segoe UI,Arial,sans-serif;padding:26px;color:#1f2328}table{width:100%;border-collapse:collapse;font-size:13px}th,td{border:1px solid #999;padding:6px 8px;text-align:left}.r{text-align:right}.c{text-align:center}.hd td{border:0;padding:2px 0}.gc{margin-top:12px}.lbl{font-size:12px;color:#555}</style></head><body>' + letterheadHtml(school, settings) + '<table class=hd><tr><td>Name: <b>' + esc(student ? student.full_name : '') + '</b></td><td>Grade: <b>' + esc(cname) + '</b></td><td>Term: <b>' + esc(term) + '</b></td></tr><tr><td>Class average: <b>' + avg + '%</b></td><td>Position in class: <b>' + (position != null ? position + ' out of ' + outOf : 'ÔÇö') + '</b></td><td>Attendance: <b>' + att.attended + ' out of ' + att.total + '</b></td></tr></table>' + '<table style="margin-top:10px"><thead><tr><th>Learning Area</th><th class=r>PupilÔÇÖs Mark</th><th class=r>Out of</th><th class=c>' + gradeLabel + '</th><th>Comment</th></tr></thead><tbody>' + body + '<tr><td><b>TOTAL</b></td><td class=r><b>' + totMark + '</b></td><td class=r><b>' + totOut + '</b></td><td class=c><b>' + (level === 'primary' ? totUnits : '') + '</b></td><td></td></tr></tbody></table>' + conductTable + '<div class=gc><span class=lbl>General comments:</span><br>' + esc(meta.general_comment || '') + '</div>' + '<div class=gc><span class=lbl>HeadÔÇÖs comments:</span><br>' + esc(meta.head_comment || '') + '</div>' + '<div class=gc><span class=lbl>Next term begins:</span> ' + esc(meta.next_term || '______________') + '</div>' + sigBlock() + stampBox() + footerHtml(NOTE) + '</body></html>';
    openPrint(html);
  }
  function printMid() {
  ...
  return (<div>
    ...
    <div className="card" style={{ marginBottom: 16 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginTop: 10 }}>
        <div><label style={labelStyle}>HeadÔÇÖs comment</label><input style={inputStyle} value={meta.head_comment} onChange={e => setMeta(m => ({ ...m, head_comment: e.target.value }))} placeholder="e.g. Pull up for better grades" /></div>
      ...
      <div className="card">
        ...
        {rows.length === 0 ? <p className="muted">No marks for this term yet.</p> : (<>
          <table><thead><tr><th>Subject</th><th className="r">Mark</th><th className="r">{level === 'primary' ? 'Units' : 'Grade'}</th><th>Comment</th></tr></thead><tbody>
            {rows.map((r, i) => (<tr key={i}><td className="strong">{r.name}</td><td className="r">{r.score}</td><td className="r" style={{ fontWeight: 700 }}>{r.grade}</td><td className="muted">{r.comment || 'ÔÇö'}</td></tr>))}
          </tbody></table>
          <div style={{ marginTop: 14, fontWeight: 700 }}>Average: {avg}%{position != null ? '  ┬À  Position: ' + position + ' of ' + outOf : ''}</div>
        </>)}
      </div>)}
  </div>);
}
...
function SchoolSettingsPanel({ schoolId, school, settings, onChange }) {
  ...
  function set(k, v) { setF(o => ({ ...o, [k]: v })); }
  function onLogo(e) { const file = e.target.files[0]; if (!file) return; if (file.size > 300000) { setErr('Logo too large ÔÇö use an image under ~300KB.'); return; } setErr(''); const r = new FileReader(); r.onload = () => set('logo', r.result); r.readAsDataURL(file); }
  async function save() {
    setBusy(true); setErr(''); setSaved(false);
  ...
  return (<div>
    ...
    <div className="card" style={{ maxWidth: 640 }}>
      <div style={{ display: 'grid', gap: 12, gridTemplateColumns: '1fr 1fr' }}>
        ...
        <div><label style={labelStyle}>School level</label><select style={inputStyle} value={f.level} onChange={e => set('level', e.target.value)}><option value="secondary">Secondary (AÔÇôF grades)</option><option value="primary">Primary (units)</option></select></div>
      ...
      {err && <p className="error">{err}</p>}
      <div style={{ marginTop: 16 }}><button onClick={save} disabled={busy}>{busy ? 'SavingÔÇª' : (saved ? 'Saved Ô£ô' : 'Save letterhead')}</button></div>
    </div>
  </div>);
}
...
function FinancePanel({ schoolId }) {
  ...
  return (<div>
    ...
    <div className="card" style={{ marginBottom: 18 }}>
      ...
      <div style={{ marginTop: 12 }}><button onClick={add} disabled={busy}>{busy ? 'SavingÔÇª' : 'Add entry'}</button></div>
      {err && <p className="error">{err}</p>}
    </div>
    <table><thead><tr><th>Date</th><th>Type</th><th>Category</th><th>Description</th><th className="r">Amount</th><th></th></tr></thead><tbody>
      {rows.map(r => (<tr key={r.id}><td>{r.date}</td><td style={{ color: r.kind === 'income' ? '#1a7f5a' : '#c0392b', fontWeight: 600, textTransform: 'capitalize' }}>{r.kind}</td><td>{r.category || 'ÔÇö'}</td><td className="muted">{r.description || 'ÔÇö'}</td><td className="r">{money(r.amount)}</td><td className="r"><button className="ghost" style={{ padding: '4px 10px', fontSize: 13 }} onClick={() => remove(r.id)}>Remove</button></td></tr>))}
...
function InventoryPanel({ schoolId, school, settings }) {
  ...
  return (<div>
    ...
    <div className="card" style={{ marginBottom: 18 }}>
      ...
      <div style={{ marginTop: 12 }}><button onClick={add} disabled={busy}>{busy ? 'SavingÔÇª' : 'Add item'}</button></div>
      {err && <p className="error">{err}</p>}
    </div>
...
function AssetsPanel({ schoolId, school, settings }) {
  ...
  return (<div>
    ...
    <div className="card" style={{ marginBottom: 18 }}>
      ...
      <div style={{ marginTop: 12 }}><button onClick={add} disabled={busy}>{busy ? 'SavingÔÇª' : 'Add asset'}</button></div>
      {err && <p className="error">{err}</p>}
    </div>
...
function DashboardPanel({ schoolId, school }) {
  ...
  const money = n => '$' + Number(n || 0).toLocaleString();
  if (!d) return <p className="muted">LoadingÔÇª</p>;
  ...
  return (<div>
    ...
    <StatRow items={[{ value: d.students, label: 'Students' }, { value: d.classes, label: 'Classes' }, { value: d.teachers, label: 'Teachers' }, { value: attPct != null ? attPct + '%' : 'ÔÇö', label: 'Attendance today' }]} />
    ...
    {d.low > 0 && <div style={{ background: '#fff8e1', border: '1px solid #f4d58a', color: '#8a6d1a', padding: '10px 14px', borderRadius: 8, fontSize: 14, marginBottom: 16 }}>{d.low} inventory item(s) low on stock ÔÇö check the Inventory tab.</div>}
    <div className="card">
...
function AnnouncementsPanel({ schoolId, canPost }) {
  ...
  return (<div>
    {canPost && (<div className="card" style={{ marginBottom: 18 }}>
      ...
      <div style={{ marginTop: 12 }}><button onClick={post} disabled={busy}>{busy ? 'PostingÔÇª' : 'Post'}</button></div>
      {err && <p className="error">{err}</p>}
    </div>)}
...
function StaffPanel({ schoolId }) {
  ...
  return (<div>
    ...
    <div className="card" style={{ marginBottom: 18 }}>
      ...
      <div style={{ marginTop: 12 }}><button onClick={add} disabled={busy}>{busy ? 'SavingÔÇª' : 'Add staff'}</button></div>
      {err && <p className="error">{err}</p>}
    </div>
    <table><thead><tr><th>Name</th><th>Role</th><th>Department</th><th>Phone</th><th>Email</th><th></th></tr></thead><tbody>
      {rows.map(r => (<tr key={r.id}><td className="strong">{r.full_name}</td><td>{r.role || 'ÔÇö'}</td><td>{r.department || 'ÔÇö'}</td><td>{r.phone || 'ÔÇö'}</td><td className="muted">{r.email || 'ÔÇö'}</td><td className="r"><button className="ghost" style={{ padding: '4px 10px', fontSize: 13 }} onClick={() => remove(r.id)}>Remove</button></td></tr>))}
...
function AdmissionsPanel({ schoolId, classes }) {
  ...
  useEffect(() => { load(); }, [schoolId]);
  function set(k, v) { setF(o => ({ ...o, [k]: v })); }
  const clsName = id => { const c = classes.find(x => x.id === id); return c ? c.name : 'ÔÇö'; };
  async function add() { if (!f.full_name.trim()) { setErr('Enter a name.'); return; } setBusy(true); setErr('');
  ...
  return (<div>
    <div className="card" style={{ marginBottom: 18 }}>
      ...
      <div style={{ display: 'grid', gap: 10, gridTemplateColumns: 'repeat(4, 1fr)' }}>
        ...
        <div><label style={labelStyle}>Class applying for</label><select style={inputStyle} value={f.class_id} onChange={e => set('class_id', e.target.value)}><option value="">ÔÇö</option>{classes.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select></div>
      </div>
      <div style={{ marginTop: 12 }}><button onClick={add} disabled={busy}>{busy ? 'SavingÔÇª' : 'Add application'}</button></div>
      {err && <p className="error">{err}</p>}
    </div>
    <table><thead><tr><th>Applicant</th><th>Guardian</th><th>Class</th><th>Status</th><th></th><th></th></tr></thead><tbody>
      {rows.map(r => (<tr key={r.id}><td className="strong">{r.full_name}</td><td>{r.guardian_name || 'ÔÇö'}{r.guardian_phone ? ' ┬À ' + r.guardian_phone : ''}</td><td>{clsName(r.class_id)}</td>
  ...
  return (<div>
    ...
    {view === 'week' ? (
      <table><thead><tr><th style={{ width: 70 }}>Period</th>{days.map(d => <th key={d}>{d}</th>)}</tr></thead><tbody>
        {periods.map(pd => (<tr key={pd}><td className="strong">{pd}</td>{days.map(d => { const c = cell(d, pd); return (<td key={d}>{c && c.subject ? <div>{c.subject}</div> : <span className="muted">ÔÇö</span>}{c && c.teacher ? <div className="muted" style={{ fontSize: 12 }}>{c.teacher}</div> : null}</td>); })}</tr>))}
      </tbody></table>
    ) : (<>
      ...
      {clashes.length > 0 && <div style={{ background: '#fdeaea', border: '1px solid #f3c2c2', color: '#c0392b', padding: '10px 14px', borderRadius: 8, fontSize: 14, marginBottom: 12 }}>{clashes.length} clash(es): a teacher is double-booked in another class at the same period ÔÇö fix before this timetable is final.</div>}
      ...
      <div style={{ marginTop: 16 }}><button onClick={save} disabled={busy}>{busy ? 'SavingÔÇª' : (saved ? 'Saved Ô£ô' : 'Save ' + day + ' timetable')}</button></div>
    </>)}
  </div>);
}
...
function TeacherDashboardPanel({ schoolId, classes, session }) {
  ...
  return (<div>
    ...
    <StatRow items={[{ value: classes.length, label: 'My classes' }, { value: d.students, label: 'My students' }, { value: attPct != null ? attPct + '%' : 'ÔÇö', label: 'TodayÔÇÖs attendance' }]} />
    <div className="card" style={{ marginBottom: 16 }}>
      <div style={{ fontWeight: 700, marginBottom: 8 }}>My classes</div>
      {classes.length === 0 ? <p className="muted">No classes assigned yet ÔÇö ask your admin.</p> : classes.map(c => <div key={c.id} style={{ padding: '6px 0', borderBottom: '1px solid #eee' }}>{c.name}</div>)}
    </div>
    <div className="card">
...
function AcademicsPanel({ schoolId, classes, subjects }) {
  ...
  return (<div>
    ...
    {loading ? <p className="muted">LoadingÔÇª</p> : (<>
      <div style={{ fontWeight: 700, marginBottom: 8 }}>All classes ┬À {term}</div>
      ...
      <table><thead><tr><th>Class</th><th className="r">Students</th><th className="r">Avg exam mark</th><th className="r">Attendance</th><th></th></tr></thead><tbody>
        {classRows.map(r => (<tr key={r.c.id} onClick={() => setOpenClass(openClass === r.c.id ? null : r.c.id)} style={{ cursor: 'pointer', background: openClass === r.c.id ? '#eafaf3' : 'transparent' }}>
          ...
          <td className="r" style={{ fontWeight: 600, color: clr(r.avg) }}>{r.avg != null ? r.avg + '%' : 'ÔÇö'}</td>
          <td className="r" style={{ fontWeight: 600, color: clr(r.attPct) }}>{r.attPct != null ? r.attPct + '%' : 'ÔÇö'}</td>
      ...
      {detail && (<div style={{ marginTop: 20 }}>
        <div style={{ fontWeight: 700, marginBottom: 8 }}>{(classes.find(c => c.id === openClass) || {}).name} ÔÇö subject averages</div>
        ...
        <div style={{ fontWeight: 700, margin: '18px 0 8px' }}>Pupils ┬À ranked by average</div>
        <table><thead><tr><th>#</th><th>Student</th><th className="r">Average</th><th className="r">Attendance</th></tr></thead><tbody>
          {detail.rows.map((r, i) => (<tr key={i}><td className="muted">{i + 1}</td><td className="strong">{r.name}</td><td className="r" style={{ fontWeight: 600, color: clr(r.avg) }}>{r.avg != null ? Math.round(r.avg) + '%' : 'ÔÇö'}</td><td className="r" style={{ color: clr(r.att) }}>{r.att != null ? r.att + '%' : 'ÔÇö'}</td></tr>))}
        </tbody></table>
      </div>)}
    </>)}
  </div>);
...
function FeeCollect({ schoolId, classId, term, className, school, settings }) {
  ...
  async function recordPay() {
    ...
    await supabase.from('finance_entries').insert({ school_id: schoolId, date: pay.paid_on, kind: 'income', category: 'Fees', description: (st ? st.full_name : '') + ' ┬À ' + term, amount: Number(pay.amount), fee_payment_id: ins ? ins.id : null });
  ...
  function printDoc(kind, student, lastPayment) {
    ...
    if (kind === 'receipt' && lastPayment) {
      inner = '<h3 style="margin:0 0 8px">Receipt</h3><div class=m>' + esc(student.full_name) + ' ┬À ' + esc(className) + ' ┬À ' + esc(term) + '</div><table style="margin-top:12px;max-width:380px"><tbody><tr><td>Date</td><td class=r>' + esc(lastPayment.paid_on || '') + '</td></tr><tr><td>Method</td><td class=r>' + esc(lastPayment.method || '') + '</td></tr>' + (lastPayment.reference ? '<tr><td>Reference</td><td class=r>' + esc(lastPayment.reference) + '</td></tr>' : '') + '<tr><td><b>Amount paid</b></td><td class=r><b>' + money(lastPayment.amount) + '</b></td></tr><tr><td>Balance after</td><td class=r>' + money(bal) + '</td></tr></tbody></table>';
    } else {
      inner = '<h3 style="margin:0 0 8px">Fee statement</h3><div class=m>' + esc(student.full_name) + ' ┬À ' + esc(className) + ' ┬À ' + esc(term) + '</div><table style="margin-top:12px"><thead><tr><th>Fee item</th><th class=r>Amount</th></tr></thead><tbody>' + itemRows + '<tr><td><b>Total due</b></td><td class=r><b>' + money(due) + '</b></td></tr></tbody></table><table style="margin-top:14px"><thead><tr><th>Payment date</th><th>Method</th><th>Reference</th><th class=r>Amount</th></tr></thead><tbody>' + (payRows || '<tr><td colspan=4>No payments yet</td></tr>') + '<tr><td colspan=3><b>Total paid</b></td><td class=r><b>' + money(paid) + '</b></td></tr></tbody></table><div style="margin-top:14px;font-weight:700">Balance: ' + money(bal) + '</div>';
    }
  ...
  return (<div>
    {due === 0 && <div style={{ background: '#fff8e1', border: '1px solid #f4d58a', color: '#8a6d1a', padding: '10px 14px', borderRadius: 8, fontSize: 14, marginBottom: 14 }}>No fees set for this class/term yet ÔÇö use "Set fees" to add them.</div>}
    ...
    {openStudent && (<div className="card" style={{ marginTop: 16 }}>
      ...
      <div className="muted" style={{ fontSize: 13, margin: '2px 0 12px' }}>Due {money(due)} ┬À Paid {money(paidOf(openStudent.id))} ┬À Balance {money(due - paidOf(openStudent.id))}</div>
      ...
      <div style={{ marginTop: 12 }}><button onClick={recordPay} disabled={busy}>{busy ? 'SavingÔÇª' : 'Record payment'}</button></div>
      {err && <p className="error">{err}</p>}
      {myPayments.length > 0 && (<table style={{ marginTop: 16 }}><thead><tr><th>Date</th><th>Method</th><th>Reference</th><th className="r">Amount</th><th></th></tr></thead><tbody>
        {myPayments.map(p => (<tr key={p.id}><td>{p.paid_on}</td><td style={{ textTransform: 'capitalize' }}>{p.method}</td><td className="muted">{p.reference || 'ÔÇö'}</td><td className="r">{money(p.amount)}</td><td className="r"><button className="ghost" style={{ padding: '3px 9px', fontSize: 12 }} onClick={() => delPay(p.id)}>Delete</button></td></tr>))}
      </tbody></table>)}
    </div>)}
  </div>);
}
...
function BankingPanel({ schoolId }) {
  ...
  return (<div>
    ...
    <div className="card" style={{ marginBottom: 18 }}>
      ...
      <div style={{ marginTop: 12 }}><button onClick={add} disabled={busy}>{busy ? 'SavingÔÇª' : 'Record'}</button></div>
      {err && <p className="error">{err}</p>}
    </div>
    <table><thead><tr><th>Date</th><th>Type</th><th>Note</th><th className="r">Amount</th><th></th></tr></thead><tbody>
      {txns.map(t => (<tr key={t.id}><td>{t.date}</td><td className="strong">{label(t.type)}</td><td className="muted">{t.note || 'ÔÇö'}</td><td className="r">{money(t.amount)}</td><td className="r"><button className="ghost" style={{ padding: '4px 10px', fontSize: 13 }} onClick={() => remove(t.id)}>Remove</button></td></tr>))}
...
function ArrearsPanel({ schoolId, classes, school, settings }) {
  ...
  return (<div>
    ...
    {loading ? <p className="muted">LoadingÔÇª</p> : (<>
      <div className="muted" style={{ marginBottom: 10, fontSize: 14 }}>{rows.length} pupil(s) owing ┬À Total owed: <b>{money(totalOwed)}</b></div>
      <table><thead><tr><th>Student</th><th>Class</th><th className="r">Due</th><th className="r">Paid</th><th className="r">Balance</th></tr></thead><tbody>
        ...
        {rows.length === 0 && <tr><td colSpan="5" className="muted">No arrears ÔÇö everyone is paid up for this term (or fees not set yet).</td></tr>}
      </tbody></table>
    </>)}
  </div>);
}
...
function FeeArrears({ schoolId, term, classes, school, settings }) {
  ...
  const money = n => '$' + Number(n || 0).toLocaleString();
  const clsName = id => { const c = classes.find(x => x.id === id); return c ? c.name : 'ÔÇö'; };
  if (rows === null) return <p className="muted">LoadingÔÇª</p>;
  ...
  return (<div>
    ...
    <table><thead><tr><th>Student</th><th>Class</th><th className="r">Due</th><th className="r">Paid</th><th className="r">Balance</th></tr></thead><tbody>
      ...
      {rows.length === 0 && <tr><td colSpan="5" className="muted">No arrears ÔÇö everyone is paid up for {term}.</td></tr>}
    </tbody></table>
  </div>);
}
  ...
  return (<div>
    ...
    <table><thead><tr><th>School</th><th className="r">Amount / mo</th><th>Next due</th><th>Status</th><th></th></tr></thead><tbody>
      {rows.map(r => { const st = status(r.sub); const isE = edit === r.id; return (<tr key={r.id}>
        ...
        <td className="r">{isE ? <input style={{ ...inputStyle, margin: 0, width: 90 }} value={form.amount} onChange={e => setForm(f => ({ ...f, amount: e.target.value }))} /> : (r.sub ? money(r.sub.amount) : 'ÔÇö')}</td>
        <td>{isE ? <input type="date" style={{ ...inputStyle, margin: 0 }} value={form.next_due} onChange={e => setForm(f => ({ ...f, next_due: e.target.value }))} /> : ((r.sub && r.sub.next_due) || 'ÔÇö')}</td>
...
function SchoolBillingPanel({ schoolId }) {
  ...
  const si = (() => {
    ...
    return { t: 'Active ┬À renews in ' + daysLeft + ' day(s)', c: '#1a7f5a' };
  })();
  ...
  if (sub === undefined) return <p className="muted">LoadingÔÇª</p>;
  return (<div>
    <div className="card" style={{ maxWidth: 520, marginBottom: 18 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, fontSize: 14 }}>
        <div><div className="muted" style={{ fontSize: 12 }}>Amount</div><div style={{ fontWeight: 600 }}>{sub && sub.amount ? money(sub.amount) + ' / month' : 'ÔÇö'}</div></div>
        <div><div className="muted" style={{ fontSize: 12 }}>Next due</div><div style={{ fontWeight: 600 }}>{(sub && sub.next_due) || 'ÔÇö'}</div></div>
        <div><div className="muted" style={{ fontSize: 12 }}>Last paid</div><div>{(sub && sub.last_paid) || 'ÔÇö'}</div></div>
      </div>
      <div style={{ marginTop: 16 }}><button onClick={payNow} disabled={busy || !sub || !sub.amount}>{busy ? 'StartingÔÇª' : 'Pay now'}</button></div>
      {msg && <p className="error">{msg}</p>}
      {(!sub || !sub.amount) && <p className="muted" style={{ fontSize: 13, marginTop: 8 }}>No amount set yet ÔÇö the platform admin will set your subscription.</p>}
    </div>
    ...
    <table><thead><tr><th>Date</th><th>Method</th><th className="r">Amount</th></tr></thead><tbody>
      {pays.map(pp => (<tr key={pp.id}><td>{pp.paid_on}</td><td style={{ textTransform: 'capitalize' }}>{pp.method || 'ÔÇö'}</td><td className="r">{money(pp.amount)}</td></tr>))}
