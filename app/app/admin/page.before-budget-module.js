    ...
    <div>
      ...
      <div className="card" style={{ marginBottom: 18 }}>
        ...
        <select style={inputStyle} value={selectedMeeting} onChange={e=>setSelectedMeeting(e.target.value)}>
          ...
          {meetings.map(m=><option key={m.id} value={m.id}>{m.meeting_date} Ã¢â‚¬â€ {m.title}</option>)}
        </select>
      ...
      {meetings.map(m=>(
        <div className="card" key={m.id} style={{marginBottom:14}}>
          ...
          <div className="muted">{m.meeting_date} Ã‚Â· {m.meeting_type} Ã‚Â· {m.venue || 'Venue not set'}</div>
          ...
          <div style={{marginTop:10}}>
            {resolutions.filter(r=>r.meeting_id===m.id).map(r=>(
              <div key={r.id} style={{padding:'10px 0',borderTop:'1px solid #e5e7eb'}}>
                ...
                <div className="muted">Owner: {r.responsible_person || '-'} Ã‚Â· Due: {r.due_date || '-'} Ã‚Â· Status: {r.status}</div>
  ...
  return <div>
    ...
    <div style={{display:'grid',gap:12}}>
      {events.map(e=><article className="card" key={e.id}>
        ...
        <div className="muted">{e.start_date}{e.end_date ? ` to ${e.end_date}` : ''} Ã‚Â· {e.category} Ã‚Â· {e.venue || 'Venue not set'}</div>
    ...
    <div style={{display:'grid',gap:12}}>
      {contractors.map(c=>{
        ...
        return <article className="card" key={c.id}>
          ...
          <div className="muted">{c.company_name||''} Ã‚Â· {c.service_type} Ã‚Â· {c.status}</div>
          <div style={{marginTop:8}}>Contract value: {c.contract_value!=null?Number(c.contract_value).toLocaleString():'Not set'} Ã‚Â· Paid: {total.toLocaleString()} Ã‚Â· Balance: {balance!=null?balance.toLocaleString():'-'}</div>
        </article>
      })}
    </div>
  </div>
  ...
  return <div>
    ...
    {tab==='invoices'?<div>
      ...
      {invoices.map(row=><article className="card" key={row.id} style={{marginBottom:12}}>
        <strong>{row.invoice_number}</strong> â€” {row.issued_to}
        <div className="muted">{row.invoice_date} Â· {row.currency} {Number(row.total).toFixed(2)} Â· Balance {row.currency} {Number(row.balance).toFixed(2)} Â· {row.status}</div>
    ...
    {tab==='receipts'?<div>
      <div className="card" style={{marginBottom:18}}>
        ...
        <select style={{...inputStyle,marginTop:10}} value={receipt.invoice_id} onChange={e=>setReceipt(x=>({...x,invoice_id:e.target.value}))}>
          ...
          {invoices.filter(i=>i.status!=='paid'&&i.status!=='void').map(i=><option key={i.id} value={i.id}>{i.invoice_number} â€” {i.issued_to}</option>)}
        </select>
        <div style={{display:'grid',gridTemplateColumns:'repeat(2,minmax(0,1fr))',gap:10,marginTop:10}}>
          {[
      ...
      {receipts.map(row=><article className="card" key={row.id} style={{marginBottom:12}}>
        <strong>{row.receipt_number}</strong> â€” {row.received_from}
        <div className="muted">{row.receipt_date} Â· {row.currency} {Number(row.amount).toFixed(2)} Â· {row.payment_method || '-'}</div>
        <div style={{display:'flex',gap:8,marginTop:10,flexWrap:'wrap'}}>
