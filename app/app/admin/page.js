    ...
    <div>
      ...
      <div className="card" style={{ marginBottom: 18 }}>
        ...
        <select style={inputStyle} value={selectedMeeting} onChange={e=>setSelectedMeeting(e.target.value)}>
          ...
          {meetings.map(m=><option key={m.id} value={m.id}>{m.meeting_date} â€” {m.title}</option>)}
        </select>
      ...
      {meetings.map(m=>(
        <div className="card" key={m.id} style={{marginBottom:14}}>
          ...
          <div className="muted">{m.meeting_date} Â· {m.meeting_type} Â· {m.venue || 'Venue not set'}</div>
          ...
          <div style={{marginTop:10}}>
            {resolutions.filter(r=>r.meeting_id===m.id).map(r=>(
              <div key={r.id} style={{padding:'10px 0',borderTop:'1px solid #e5e7eb'}}>
                ...
                <div className="muted">Owner: {r.responsible_person || '-'} Â· Due: {r.due_date || '-'} Â· Status: {r.status}</div>
  ...
  return <div>
    ...
    <div style={{display:'grid',gap:12}}>
      {events.map(e=><article className="card" key={e.id}>
        ...
        <div className="muted">{e.start_date}{e.end_date ? ` to ${e.end_date}` : ''} Â· {e.category} Â· {e.venue || 'Venue not set'}</div>
    ...
    <div style={{display:'grid',gap:12}}>
      {contractors.map(c=>{
        ...
        return <article className="card" key={c.id}>
          ...
          <div className="muted">{c.company_name||''} Â· {c.service_type} Â· {c.status}</div>
          <div style={{marginTop:8}}>Contract value: {c.contract_value!=null?Number(c.contract_value).toLocaleString():'Not set'} Â· Paid: {total.toLocaleString()} Â· Balance: {balance!=null?balance.toLocaleString():'-'}</div>
        </article>
      })}
    </div>
  </div>
